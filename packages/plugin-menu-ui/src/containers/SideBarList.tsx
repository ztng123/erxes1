...
const TypesListContainer = (props: FinalProps) => {
  const { listMenuTypeQuery, typesEdit, typesRemove, history } = props;

  if (listMenuTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listMenuTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...
    loading: listMenuTypeQuery.loading,
    remove,
    renderButton
  };
