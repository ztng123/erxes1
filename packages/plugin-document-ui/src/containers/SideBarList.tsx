...
const TypesListContainer = (props: FinalProps) => {
  const { listDocumentTypeQuery, typesEdit, typesRemove, history } = props;

  if (listDocumentTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listDocumentTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...
    loading: listDocumentTypeQuery.loading,
    remove,
    renderButton
  };
